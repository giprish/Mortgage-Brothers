...
const url = "https://azmortgagebrothers.com/videos/mortgage-loan-products/";
  ...
  .get(url, (res) => {
    ...
    res.on("end", () => {
      ...
      for (const x of embeds) {
        ...
        seen.add(youtubeId);
        videos.push({
          id: `products-${String(videos.length + 1).padStart(2, "0")}`,
          title: decode(x[1]),
          youtubeId,
        });
      }
      ...
      fs.writeFileSync(
        "app/videos/mortgage-loan-products-videos.ts",
        `import type { VideoItem } from "./videos-data";

/** Full Mortgage Loan Products library from the live site. */
export const MORTGAGE_LOAN_PRODUCTS_FULL_VIDEOS: VideoItem[] = ${JSON.stringify(videos, null, 2)};
`
      );
