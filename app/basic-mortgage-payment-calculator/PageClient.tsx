...
import { useMemo, useState, useId, cloneElement, isValidElement, type ReactNode } from "react";
...
function Field({ label, children, error, hint }: { label: string; children: ReactNode; error?: string | null; hint?: string }) {
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <label htmlFor={id} style={{ display: "block", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".05em", color: C.inkSoft, marginBottom: 7 }}>
        {label}
      </label>
      {control}
...
function MoneyInput({ value, onChange, onBlur, id }: { value: string; onChange: (v: string) => void; onBlur: () => void; id?: string }) {
  return (
    <div style={{ position: "relative" }}>
      ...
      <input id={id} className="dpc-input" style={{ ...baseInput, padding: "10px 12px 10px 26px" }} inputMode="decimal" value={value}
        onChange={(e) => onChange(e.target.value)} onBlur={onBlur} />
    </div>
  );
}
function RateInput({ value, onChange, onBlur, id }: { value: string; onChange: (v: string) => void; onBlur: () => void; id?: string }) {
  return (
    <div style={{ position: "relative" }}>
      <input id={id} className="dpc-input" style={{ ...baseInput, padding: "10px 26px 10px 12px" }} inputMode="decimal" value={value}
        onChange={(e) => onChange(e.target.value)} onBlur={onBlur} />
...
function MiniSlider({ min, max, step, value, onChange, id }: { min: number; max: number; step: number; value: number; onChange: (v: number) => void; id?: string }) {
  const safe = isFinite(value) ? Math.min(max, Math.max(min, value)) : min;
  return (
    <input id={id} type="range" min={min} max={max} step={step} value={safe}
    ...
    <div style={{ background: "#f7f8f5", border: `1px solid ${C.line}`, borderRadius: 10, boxShadow: "0 1px 2px rgba(28,42,23,0.06), 0 6px 20px rgba(28,42,23,0.05)", padding: 22, marginTop: 24, marginBottom: 20 }}>
      ...
      <div className="insights-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.3fr) minmax(0, 1fr)", gap: 20, alignItems: "start" }}>
        <div>
          {groups.map((g) => (
            <div key={g.title} style={{ marginBottom: 16 }}>
              <h3 style={{ fontSize: 13.5, fontWeight: 700, margin: "0 0 8px", color: g.color }}>{g.title}</h3>
        ...
        <div style={{ background: "#fff", border: `1px solid ${C.line}`, borderRadius: 8, padding: 16 }}>
          <h3 style={{ fontSize: 13.5, fontWeight: 700, margin: "0 0 4px", color: C.ink }}>Next Steps</h3>
    ...
    <div className="flex flex-col min-h-screen" style={{ background: C.paper }}>
      ...
      <main className="flex-grow overflow-x-hidden" style={{ color: C.ink, fontFamily: SANS, fontSize: 15, lineHeight: 1.5, overflowX: "hidden" }}>
        <LoanProgramHero
          ...
          subtitle="Adjust the inputs below to see your monthly payment, amortization schedule, and total cost breakdown — results update instantly."
          ctaLabel=""
          note=""
        />
