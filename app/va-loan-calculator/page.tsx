...
export default function VaLoanCalculatorPage() {
  ...
  const [result, setResult] = useState<VaCalcResult | null>(null);
  ...
  const handleDpAmtChange = (val: string) => {
    ...
    if (hp > 0) {
      setDpPct(((parseFloat(val) || 0) / hp * 100).toFixed(2));
    }
  };
  ...
  const handleDpPctChange = (val: string) => {
    ...
    const hp = parseFloat(homePrice) || 0;
    if (hp > 0) {
      setDpVal(String(Math.round(hp * (parseFloat(val) || 0) / 100)));
    }
  };
  ...
  const handlePresetDp = (pct: number) => {
    ...
    const hp = parseFloat(homePrice) || 0;
    if (hp > 0) {
      setDpVal(String(Math.round(hp * pct / 100)));
    }
  };
  ...
  const handleCalculate = () => {
    ...
    const dpP = parseFloat(dpPct) || 0;
    const rate = parseFloat(interestRate) || 0;
    let term = Math.round(parseFloat(loanTerm) || 30);
    term = Math.max(1, Math.min(30, term));

    const r = solveVaPayments(hp, dp, dpP, fundingFeeStatus, loanType, rate, term);
    setResult(r);
    setCurrentPage(0);
  ...
  const evaluateEligibility = () => {
    ...
    if (!eraConfig) return "";
    const months = parseInt(serviceMonths) || 0;
  ...
  const getDonutSplit = (r: VaCalcResult) => {
    ...
    const int1 = r.totalLoanAmount * (parseFloat(interestRate) / 100 / 12);
    const p1 = total - int1;
    return {
      ...
      <main className="flex-grow">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-6 flex flex-col gap-6">
              ...
              <div className="bg-white rounded-3xl border border-[#e8e0d0]/60 p-6 lg:p-8 shadow-sm flex flex-col gap-5">
                ...
                <div>
                  ...
                  <input type="number" value={serviceMonths} onChange={(e) => setServiceMonths(e.target.value)}
            ...
            <div className="lg:col-span-6 flex flex-col gap-6">
              ...
              <div className="bg-white rounded-3xl border border-[#e8e0d0]/60 p-6 lg:p-8 shadow-sm flex flex-col gap-5">
                ...
                <div>
                  <label className="text-[#052316] text-[13.5px] font-semibold block mb-1.5">
                    {loanType === "purchase" ? "Home Price ($)" : "Home Value ($)"}
                  </label>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2 mb-3">
                    {[0, 3.5, 5, 10, 20].map((pVal) => (
                      <button key={pVal} onClick={() => handlePresetDp(pVal)}
                        className={`py-2 px-1 text-[11px] font-bold border rounded-xl transition-all cursor-pointer ${parseFloat(dpPct) === pVal ? "bg-[#052316] text-white border-[#052316]" : "bg-[#fcf9f3] text-[#052316] border-[#e8e0d0]"}`}>
                        {pVal}%
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative">
                      ...
                      <input type="number" value={dpVal} onChange={(e) => handleDpAmtChange(e.target.value)}
                    ...
                    <div className="relative">
                      <input type="number" step="0.01" value={dpPct} onChange={(e) => handleDpPctChange(e.target.value)}
        ...
        {result && (
          <section id="calc-results" className="pb-16 px-6 lg:px-10 max-w-6xl mx-auto space-y-8 animate-fadeUp font-sans">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-3xl border border-[#e8e0d0]/60 p-6 shadow-sm flex flex-col justify-between items-center text-center">
                <div className="w-full text-left">
                  <h3 className="text-[#052316] text-[16px] font-bold pb-3 border-b border-[#e8e0d0]/40 font-sans font-sans">Payment Allocation (Month 1)</h3>
                </div>
            ...
            {/* Dynamic Recommendations & Insights (Section 10) */}
            <div className="bg-white rounded-3xl border border-[#e8e0d0]/60 p-6 shadow-sm space-y-4">
              ...
              <div className="flex flex-col gap-3 font-sans text-[13.5px] leading-relaxed text-[#4e5b4e]">
                {loanType === "purchase" && parseFloat(dpPct) < 5.0 && (
                  <p>
                ...
                {loanType === "purchase" && parseFloat(dpPct) >= 5.0 && parseFloat(dpPct) < 10.0 && (
                  <p>
        ...
        {!result && (
          <section className="pb-16 px-6 lg:px-10 max-w-3xl mx-auto text-center font-sans">
            <div className="bg-white rounded-3xl border border-[#e8e0d0]/60 p-10 lg:p-14 shadow-sm">
              ...
              <h3 className="text-[#052316] text-[20px] font-bold mb-2">Check VA Eligibility & Payments</h3>
              <p className="text-[#888] text-[14.5px] leading-relaxed max-w-md mx-auto font-sans">
                Fill in your service details and pricing expectations, then click <strong>&ldquo;Calculate VA Loan Scenarios&rdquo;</strong> to view detailed metrics.
              </p>
            </div>
          </section>
        )}
