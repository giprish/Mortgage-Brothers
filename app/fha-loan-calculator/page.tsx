...
@media (max-width: 640px) { .fha-insights-grid { grid-template-columns: minmax(0, 1fr) !important; } }`}</style>
    </div>
  );
}
    ...
    <div className="flex flex-col min-h-screen" style={{ background: C.paper }}>
      ...
      <main className="flex-grow overflow-x-hidden" style={{ color: C.ink, fontFamily: SANS, fontSize: 15, lineHeight: 1.5, overflowX: "hidden" }}>
        <LoanProgramHero
          ...
          subtitle="Estimate FHA payments, MIP, and Arizona county loan limits — results update as you adjust the inputs."
          ctaLabel=""
          note=""
        />
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "28px 20px 64px", boxSizing: "border-box" }}>
          ...
          <div className="fha-layout" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: 24, alignItems: "start" }}>
            {/* ============ INPUT COLUMN ============ */}
            <div style={{ minWidth: 0 }}>
              <Panel number={1} title="Property Details">
                <SelectField label="Arizona County" value={county} onChange={onCountyChange}
                  options={COUNTIES.map((c) => ({ value: c, label: `${c} County` }))} />
        ...
        <style>{`
          @media (max-width: 1023px) {
            .fha-layout { grid-template-columns: minmax(0, 1fr) !important; }
          }
