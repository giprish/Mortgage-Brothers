...
export default function DtiCalculatorPage() {
  // Income State
  const [primaryIncome, setPrimaryIncome] = useState<number>(6000);
  const [additionalIncome, setAdditionalIncome] = useState<number>(1000);

  // Housing Expense State
  const [mortgagePayment, setMortgagePayment] = useState<number>(1500);
  const [propertyTaxes, setPropertyTaxes] = useState<number>(200);
  ...
  const [carLoans, setCarLoans] = useState<number>(350);
  ...
  // Auto-calculated result via useMemo
  const result = useMemo(() => {
    const pInc = Math.max(0, primaryIncome || 0);
    const aInc = Math.max(0, additionalIncome || 0);
    const mort = Math.max(0, mortgagePayment || 0);
    const tax = Math.max(0, propertyTaxes || 0);
    const ins = Math.max(0, homeInsurance || 0);
    const hoaVal = Math.max(0, hoaFees || 0);
    const car = Math.max(0, carLoans || 0);
    const cc = Math.max(0, creditCards || 0);
    const student = Math.max(0, studentLoans || 0);
    const other = Math.max(0, otherDebts || 0);
    return solveDti(pInc, aInc, mort, tax, ins, hoaVal, car, cc, student, other);
  }, [primaryIncome, additionalIncome, mortgagePayment, propertyTaxes, homeInsurance, hoaFees, carLoans, creditCards, studentLoans, otherDebts]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#fcf9f3] font-sans">
      ...
      <main className="flex-grow">
        ...
        {/* Results output section — always visible */}
